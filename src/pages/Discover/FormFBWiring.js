...
const FormFBWiring = props => {
    ...
    if (fieldName === 'stagesFABDetails') {
      ...
      fabricationInfo.Panels.forEach(async item => {
        if (item.MonitoringID) {
          ...
          const _Data = await firestore().collection(MonitoringID + '/Fabrication').doc('Wiring').get();
  ...
  const handleSubmit = async () => {
    ...
    for (const value of panelsToUpdate) {
      ...
      const fabDoc = firestore().collection('Monitoring').doc(MonitoringID).collection('Fabrication').doc('Wiring');
      const validPanels = await firestore().collection('Monitoring/' + MonitoringID + '/Fabrication').doc('Wiring').get();
      const isValid = validPanels.exists;
      const invalidMsg = () => {
        return updateError(value.pnameInput+' is not valid. \n Ensure the selected panel has started wiring.', setError)
      }
        ...
        if (fabricationInfo.stagesFABDetails === 'Start') {
          ...
          operations.push(
            ...
            batch.update(projectRef, {
              status: 'Fabrication - Wiring Process',
          ...
          operations.push(
            ...
            batch.update(projectRef, {
              status: 'Fabrication - Wiring Finished',
              updatedAt: firestore.Timestamp.fromDate(date),
            }),
          );
        } 
      ...
      if (panelSelected && operations.length > 0) {
        try {
          ...
          await batch.commit();
          setIsSaving(false);
          ToastAndroid.show('Fabrication Wiring Updated',ToastAndroid.SHORT);
          if (stagesFABDetails === 'Start') {navigation.replace('WiringStart')}
          if (stagesFABDetails === 'Finish') {navigation.replace('WiringFinish')}
        } catch (error) {
    ...
    const isValidForm = () => {
      ...
      if (!stagesFABDetails)
        return updateError('Required to choice Stages of Fabrication Wiring', setError);
      if (!date)
  ...
  return (
    <ScrollView style={{marginVertical: 20}}>
      <Header /><Title2 TxtTitle="WIRING" SubTitle={'FABRICATION'} />
