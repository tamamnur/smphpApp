...
const FormFBWiring = (props) => {
  ...
  const [wiringInfo, setWiringInfo] = useState({
    projectId: '', FSProjectId: '', projectName: '', stagesFABDetail: '', customer: '', projectsList: [], Panels: []
  });
  
  const {projectId, projectName, stagesFABDetail, customer} = wiringInfo;

  const handleOnchangeText = async (value, fieldName) => {
    setWiringInfo({...wiringInfo, [fieldName]: value});
    if (fieldName === 'stagesFABDetail') {
      setIsLoading(true)
      const selectedStage = value;
      wiringInfo.Panels.forEach(async item => {
  ...
  const handleComponent = async () => {
    ...
    wiringInfo.Panels.forEach(async value => {
      if (value.selected === true) {
        ...
        if (value.MonitoringID) {
          ...
          const newMonitoring = await firestore()
            ...
            .add({
              ProjectID: '/Project/' + wiringInfo.FSProjectId,
            });
        ...
        if (wiringInfo.stagesFABDetail === 'Start') {
          await Shopdrawing.doc('Wiring').set({
            Start: firestore.Timestamp.fromDate(date),
          });
        }
        if (wiringInfo.stagesFABDetail === 'Finish') {
          await Shopdrawing.doc('Wiring').update({
  ...
  const AllPanelsExistMessage = () => {
    if (wiringInfo.Panels.every(item => item.stageExist)) {
      return (
  ...
  return (
    <View style={styles.page}>
      ...
      <Title2 TxtTitle="WIRING  -  FABRICATION" />
      {error ? (
      ...
      <ScrollView style={{marginTop: 5}}>
        {isLoading ? (
          ...
          <View>
            ...
            {wiringInfo.Panels.filter(item => !item.stageExist).map(
              item => (
                <Panel
                  key={item.id}
