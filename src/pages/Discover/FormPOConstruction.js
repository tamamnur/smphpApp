...
//     projectId: '', FSProjectId: '', projectName: '', stages: '', stagesPODetails: '', customer: '',
//     projectsList: [], Panels: [],
//   });
//   const {projectId, projectName, stagesPO, stagesPODetails, customer} = constructionInfo;
//   const handleOnchangeText = async (value, material, stages) => {
//     setConstructionInfo(prev => ({...prev, [material]: value}));
//     if (stages === 'stagesPODetails') {
//       const selectedMaterial = constructionInfo[material];
//       const selectedStage = value;
//       console.log(selectedMaterial, selectedStage)
//       const updatedPanels = await Promise.all(constructionInfo.Panels.map(async item =>{
//         if (item.MonitoringID) {
//           const MonitoringID = item.MonitoringID.substring(1);
//           const _Data = await firestore().collection(MonitoringID + '/Procurement').doc(selectedMaterial).get();
//           const isExist = _Data.exists && _Data.data() && _Data.data().hasOwnProperty(selectedStage);
//           console.log(item.pnameInput, isExist)
//           return {...item, stageExist: isExist}
//         }else{
//           return {...item, stageExist: false}
//         }
...
//             <Text style={[right, {width: '62%'}]}>{customer}</Text>
//             <Text style={[right, {width: '34%'}]}>{projectId}</Text>
//           </View>
//           <View style={{width: '98%'}}>
//             <StagesPO onValueChange={value => {handleOnchangeText(value, 'stagesPO')}}/>
//           </View>
//           <View style={{width: '98%'}}>
...
const FormPOConstruction = props => {
  ...
  const handleOnchangeText = async (value, fieldName) => {
    ...
    if (fieldName === 'stagesPODetails') {
      const docName = constructionInfo.stages;
      const selectedStage = value;
      constructionInfo.Panels.forEach(async item => {
        if (item.MonitoringID) {
          const MonitoringID = item.MonitoringID.substring(1);
          ...
          console.log(item.pnameInput, 'da',constructionInfo.stages, selectedStage, isExist)
        } else {
          setConstructionInfo(prev => ({
            ...prev, Panels: prev.Panels.map(panel => {
              if (panel.pnameInput === item.pnameInput) {
    ...
    <ScrollView style={{marginVertical: 20}}>
      ...
      <View style={{flexDirection: 'row', marginHorizontal: 10, width: '100%'}}>
        ...
        <View style={{width: '70%'}}>
          ...
          <View style={{flexDirection: 'row'}}>
            <Text style={[right, {fontSize: 14,width: '62%'}]}>{customer}</Text>
            <Text style={[right, {fontSize: 14,width: '34%'}]}>{projectId}</Text>
          </View>
          <View style={{width: '98%'}}>
            <StagesPO onValueChange={value => {handleOnchangeText(value, 'stages');console.log(value)}}/>
          </View>
          <View style={{width: '98%'}}>
            <StagesPODetail onValueChange={value => {handleOnchangeText(value, 'stagesPODetails')}}/>
          </View>
