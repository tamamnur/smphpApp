...
const ComponentRealized = () => {
          ...
          const fetchDatePromises = panelIdData.map(async panel => {
            ...
            if (getId) {
              ...
              const monitoringRef = idRef.doc(id)
                .collection('Procurement').doc('Component');
              ...
              if (monitoringDoc.exists) {
                ...
                if (monitoringData.Realized) {
                  const dateValue = monitoringData.Realized;
  ...
  return (
    <View>
      ...
      <Title2 TxtTitle="ARRIVAL REALIZATION" SubTitle="COMPONENT" />
      {isLoading ? (<></>) : (<>
...
export default ComponentRealized;