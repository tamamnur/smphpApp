...
const FormFBLayouting = props => {
  ...
  const handleSubmit = async () => {
    ...
    for (const value of panelsToUpdate) {
      ...
      const fabDoc = firestore().collection('Monitoring').doc(MonitoringID).collection('Fabrication').doc('Layouting');
      const validPanels = await firestore().collection('Monitoring/' + MonitoringID + '/Fabrication').doc('Layouting').get();
      const isValid = validPanels.exists;
      const invalidMsg = () => {
        return updateError(value.pnameInput+' is not valid. \n Ensure the selected panel has started layouting.', setError)
      }
        ...
        if (fabricationInfo.stagesFABDetails === 'Start') {
          ...
          operations.push(
            ...
            batch.update(projectRef, {
              status: 'Fabrication - Layouting Process',
          ...
          operations.push(
            ...
            batch.update(projectRef, {
              status: 'Fabrication - Layouting Finished',
              updatedAt: firestore.Timestamp.fromDate(date),
            }),
          );
        } 
      ...
      if (panelSelected && operations.length > 0) {
        try {
          ...
          await batch.commit();
          setIsSaving(false);
          ToastAndroid.show('Fabrication Layouting updated',ToastAndroid.SHORT);
          if (stagesFABDetails === 'Start') {navigation.replace('LayoutingStart')}
          if (stagesFABDetails === 'Finish') {navigation.replace('LayoutingFinish')}
        } catch (error) {
    ...
    const isValidForm = () => {
      ...
      if (!stagesFABDetails)
        return updateError('Required to choice Stages of Fabrication Layout', setError);
      if (!date)
  ...
  return (
    <ScrollView style={{marginVertical: 20}}>
      <Header /><Title2 TxtTitle="LAYOUT" SubTitle={'FABRICATION'} />
